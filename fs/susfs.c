...
void susfs_set_hide_sus_mnts_for_non_su_procs(bool enabled) {}
EXPORT_SYMBOL_GPL(susfs_set_hide_sus_mnts_for_non_su_procs);



struct susfs_sus_path_node {
	struct list_head list;
	char *path;
	unsigned long ino;
};
int susfs_sus_ino_for_filldir64(unsigned long ino) {
	struct susfs_sus_path_node *node;

	/* Se l'utente è root (UID < 2000), non nascondere nulla (per evitare crash di sistema) */
	if (current_uid().val < 2000)
		return 0;

	/* Controlla nella lista dei loop mounts (tipici di KSU) */
	spin_lock(&susfs_spin_lock_sus_path);
	list_for_each_entry(node, &LH_SUS_PATH_LOOP, list) {
		if (node->ino == ino) {
			spin_unlock(&susfs_spin_lock_sus_path);
			return 1;
		}
	}
	/* Controlla nella lista Android Data */
	list_for_each_entry(node, &LH_SUS_PATH_ANDROID_DATA, list) {
		if (node->ino == ino) {
			spin_unlock(&susfs_spin_lock_sus_path);
			return 1;
		}
	}
	/* Controlla nella lista SDCARD */
	list_for_each_entry(node, &LH_SUS_PATH_SDCARD, list) {
		if (node->ino == ino) {
			spin_unlock(&susfs_spin_lock_sus_path);
			return 1;
		}
